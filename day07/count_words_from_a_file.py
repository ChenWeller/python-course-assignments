...
def show_usage():
    """
    Show user how to run the script properly.
    """
    script_name = os.path.basename(sys.argv[0])
    print(f"\nUsage: python {script_name} path/to/textfile.txt")
    print("Example: python count_words_from_a_file.py examples/dictionary/words_and_spaces.txt\n")

def read_file_content(filepath):
    """
    Try to read the file content and handle errors.
    """
    try:
...
def display_results(counter):
    """
    Display the word frequency, sorted by most common.
    """
