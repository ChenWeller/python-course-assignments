...
# Create a reverse lookup dictionary: codon -> amino acid
codon_to_amino = {}
...
def show_usage():
    """
    Show user how to use the script properly.
    """
...
def read_sequence_from_file(path):
    """
    Read and clean DNA sequence from a text file.
    """
    try:
        with open(path, 'r', encoding='utf-8') as f:
            lines = f.readlines()
            # Remove FASTA headers and whitespace, join into one string
...
def count_amino_acids(sequence):
    """
    Convert codons to amino acids and count them.
    """
...
def display_results(counts, stop_codons):
    """
    Display amino acid counts in a user-friendly way.
    """
    print("\nAmino Acid Frequency:")
    for amino, count in sorted(counts.items()):
        print(f"{amino}: {count}")
    print(f"\nSTOP codons encountered (ignored): {stop_codons}")
